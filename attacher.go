...
func marathonAppName(container *docker.Container) string {
	for _, kv := range container.Config.Env {
		kvp := strings.SplitN(kv, "=", 2)
		if len(kvp) == 2 && kvp[0] == "MARATHON_APP_ID" {
			return kvp[1]
		}
	}
	return ""
...
func normalName(container *docker.Container) string {
	marathonId := marathonAppName(container)
	if marathonId != "" {
		return marathonId
	} else {
...
func hasIgnoreEnv(container *docker.Container) bool {
	for _, kv := range container.Config.Env {
		kvp := strings.SplitN(kv, "=", 2)
...
func ignoreContainer(container *docker.Container) bool {
	if hasIgnoreEnv(container) || marathonAppName(container) == "" {
		return true
	} else {
		return false
	}
...
func (m *AttachManager) attach(id string) {
	...
	if ignoreContainer(container) {
		debug("attacher:", id, "ignored: environ ignore")
		return
	}
	name := normalName(container)
